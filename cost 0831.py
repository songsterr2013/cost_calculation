...
def export_to_csv(data,rename): # 參數:(DATA,命名) 最後匯出成CSV檔的話都是來這一步
...
def get_material_data_and_bom(): #拉會計給我的BOM，跟從進貨表中整理好的進貨成本，去算出料錢，之後怎麼樣一直收DATA就要大改這邊了
    material_hardware = pd.read_excel('-會計統整後的原料成本.xlsx', sheet_name=1)  # 會計整理過的五金件成本
    material_hardware = material_hardware[['零件名稱', '進價']] # 只拿我要的欄位
    material_hardware.columns = ['零件名稱', '五金進價'] # 改名等下MERGE用

    material_iron = pd.read_excel('-會計統整後的原料成本.xlsx', sheet_name=3)  # 會計整理過的鈑材類成本
    material_iron = material_iron[['類別', '平均單價']] # 只拿我要的欄位
    material_iron.columns = ['產品類別', '鈑材平均單價'] # 改名等下MERGE用

    bom = pd.read_excel('-BOM表.xlsx', sheet_name=0) # 會計整理過後的BOM表，真實的BOM有60000多筆...要怎麼拿?
...
def merge_these_table_with_bom(bom,typesetting,laser,bending):

    grouped = typesetting.groupby('母件料號')  # 用groupby的方法處理
    agg = grouped['子件排版單一成本'].agg(np.sum)
    typesetting_parent_cost = pd.DataFrame(agg)
    typesetting_parent_cost.reset_index(inplace=True)

    grouped = laser.groupby('母件料號')  # 用groupby的方法處理
    agg = grouped['子件雷射單一成本'].agg(np.sum)
    laser_parent_cost = pd.DataFrame(agg)
    laser_parent_cost.reset_index(inplace=True)

    grouped = bending.groupby('母件料號')  # 用groupby的方法處理
    agg = grouped['子件折床單一成本'].agg(np.sum)
    bending_parent_cost = pd.DataFrame(agg)
    bending_parent_cost.reset_index(inplace=True)

    bom_df_with_typesetting = pd.merge(bom, typesetting_parent_cost, how='left')
    bom_df_with_typesetting_laser = pd.merge(bom_df_with_typesetting,laser_parent_cost, how='left')
    bom_df_with_typesetting_laser_bending = pd.merge(bom_df_with_typesetting_laser,bending_parent_cost, how='left')

    return bom_df_with_typesetting_laser_bending
...
if __name__ == '__main__':
    ...
    real_typesetting_cost = cost_per_one('typesetting', almost_completed_typesetting_data) # 這個之後把它刪掉

        # 4.這邊是會計叫我做的查看雷射異常DATA(有需要再用)  記得要去deal_with_abnormal()把它的星號除掉，比較方便
    # abnormal_parents(real_laser_cost)
    
        # 匯出雷射單一子件成本
    export_to_csv(real_laser_cost, '1雷射0514-0830')
    export_to_csv(real_typesetting_cost, '0排版0514-0830')    # 這個之後把它刪掉
# ===================================================以上為雷射成本的部份===================================================
    ...
    real_bending_cost = cost_per_one('bending', almost_completed_bending_data)
        ...
        # 匯出折床單一子件成本
    export_to_csv(real_bending_cost, '2折床0513-0830')
...
# ===================================================以上為折床成本的部份===================================================
    ...
    real_welding_cost = cost_per_one('welding', almost_completed_welding_data)
        ...
        # 匯出折床單一子件成本
    export_to_csv(real_welding_cost, '3焊接0602-0830')

# ===================================================以上為焊接成本的部份===================================================

    material_cost = get_material_data_and_bom()

    # 這邊是把BOM+料錢+排版子件成本+雷射子件成本+折床子件成本+焊接子件成本
    merged_cost_table = merge_these_table_with_bom(material_cost, real_typesetting_cost, real_laser_cost, real_bending_cost)
    export_to_csv(merged_cost_table, 'Bom_with_cost_from_earliest_to_0830')
